// Mock data generators for EEG MI benchmark prototype.
// All shapes mirror the FastAPI backend contract in the spec.
...
window.MOCK = (function () {
  ...
  function options() {
    return {
      ...
      subjects: Object.fromEntries(DATASETS.map(d => [d.id, Array.from({length: d.subjects}, (_, i) => i + 1)])),
      runs: Object.fromEntries(DATASETS.map(d => [d.id, Array.from({length: d.runs}, (_, i) => `run-${String(i + 1).padStart(2, "0")}`)])),
    };
  }

  // ---- Metadata --------------------------------------------------------------
  function load(dataset, subject, run) {
  ...
  // ---- Time series (C3 / C4 / Cz) -------------------------------------------
  // Realistic-looking MI epoch: ERD on contralateral side after cue (t=0).
  function timeseries(dataset, subject, run, epoch_idx, klass) {
    const sfreq = 250;
    const tmin = -0.5, tmax = 2.0;
    ...
    // class: 0=left_hand (ERD on C4), 1=right_hand (ERD on C3), 2=feet (Cz),
    // 3=tongue (broad). For demo we cycle by epoch_idx if not given.
    const cls = klass != null ? klass : (epoch_idx % 4);

    const erdOn = (t, target) => {
      // attenuation factor: 1.0 baseline, drops to ~0.3 between 0.4s-1.6s on target ch
      if (t < 0) return 1;
      const center = 1.0;
      const width = 0.7;
    ...
    return {
      ...
      channels: {
        C3: chan("C3", { 0: false, 1: true,  2: false, 3: false }),
        C4: chan("C4", { 0: true,  1: false, 2: false, 3: false }),
        Cz: chan("Cz", { 0: false, 1: false, 2: true,  3: false }),
      },
  ...
  // ---- PSD -------------------------------------------------------------------
  function psd(dataset, subject, run, channel) {
    const freqs = [];
    for (let f = 1; f <= 40; f += 0.5) freqs.push(+f.toFixed(1));
    const rng = mulberry32(hash(`${dataset}|${subject}|${run}|${channel}|psd`));

    // 1/f baseline + alpha peak around 10 Hz + smaller beta peak around 22 Hz.
    // C3/C4 get attenuated alpha (mu suppression) vs Cz.
  ...
  // ---- Topoplot --------------------------------------------------------------
  // Returns an SVG string that mimics what MNE would render server-side.
  // 22-channel 10-20 layout; smooth scalar field interpolated via inverse-distance.
  function topoplot(dataset, subject, run, freq_band) {
    const W = 360, H = 360;
    const cx = W / 2, cy = H / 2;
    const R = 140; // head radius

    // Electrode positions (approx 10-20, projected to 2D unit disk then scaled)
    // x is left(-) to right(+), y is posterior(-) to anterior(+)
    const electrodes = [
      ["Fz",   0.0,  0.55],
      ["FC3", -0.40, 0.35], ["FC1", -0.18, 0.38], ["FCz", 0.0, 0.40], ["FC2", 0.18, 0.38], ["FC4", 0.40, 0.35],
      ["C5",  -0.70, 0.0],  ["C3", -0.45, 0.0],  ["C1", -0.20, 0.0], ["Cz", 0.0, 0.0],   ["C2", 0.20, 0.0],  ["C4", 0.45, 0.0],  ["C6", 0.70, 0.0],
      ["CP3", -0.42, -0.35], ["CP1", -0.20, -0.36], ["CPz", 0.0, -0.38], ["CP2", 0.20, -0.36], ["CP4", 0.42, -0.35],
      ["P1",  -0.18, -0.58], ["Pz", 0.0, -0.60], ["P2", 0.18, -0.58],
      ["POz", 0.0, -0.78],
    ].map(([n, x, y]) => ({ name: n, x: cx + x * R, y: cy - y * R }));

    // Source strengths: simulate motor-cortex bilateral activation for mu/beta,
    // occipital for "alpha" band, broad for "broadband".
    const sources = {
    ...
    }[freq_band] || [{ name:"Cz", v: 0.5 }];

    // Build value at each electrode via inverse-distance from sources
    ...
    // Sample scalar field on a grid clipped to the head circle
    const GRID = 28;
    const cells = [];
    ...
    // RdBu diverging colormap (centered at 0)
    const absMax = Math.max(Math.abs(vMin), Math.abs(vMax)) || 1;
    function color(v) {
      const t = Math.max(-1, Math.min(1, v / absMax));
      // negative = blue, positive = red (matplotlib RdBu_r flipped to RdBu)
      const stops = t < 0
        ? lerpColor([5,48,97], [247,247,247], 1 + t)   // -1..0
        : lerpColor([247,247,247], [103,0,31], t);     // 0..1
    ...
    const electrodeSvg = electrodes.map(e =>
    ...
    const labelSvg = electrodes
      ...
      .map(e => `<text x="${(e.x + 6).toFixed(2)}" y="${(e.y - 6).toFixed(2)}" font-family="var(--mono)" font-size="9" fill="var(--topo-label, #cbd5e1)">${e.name}</text>`)
      .join("");

    // Head + nose + ears outline
    const head = `
      <defs>
        <clipPath id="headClip"><circle cx="${cx}" cy="${cy}" r="${R}"/></clipPath>
      </defs>
    ...
    for (let i = 0; i <= 10; i++) {
      const t = -1 + (2 * i) / 10;
      stops.push(`<stop offset="${(i * 10)}%" stop-color="${color(t * absMax)}"/>`);
    }
    const cb = `
      <defs>
        <linearGradient id="cbGrad" x1="0" y1="1" x2="0" y2="0">${stops.join("")}</linearGradient>
      </defs>
  ...
  return { options, load, timeseries, psd, topoplot, DATASETS };
})();
